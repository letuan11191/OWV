...
<body>
 ...
 <div id="page-wrapper">
  <div class="container-fluid">
    <div class="row">
      <div class="col-lg-12">
        <h1 class="page-header"><?php echo $_GET['name'];  ?> gửi đến  </h1>
      </div>
      ...
      <form action="savepurchase.php" method="post" class = "form-group">
        ...
        ?>
        <label>From: </label><input type="text" name="from" class="form-control" style="width: 600px;">
        <label>Ngày nhận thư : </label><input type="text"  style="width: 600px;" class = "form-control" name="date" value = "<?php echo $today; ?>" />

        <label>Loại bưu phẩm : </label><input type = "text" style="width: 600px;" class = "form-control" name = "supplier" value = "<?php echo $_GET['name']; ?>">
      ...
      <label>Ghi chú</label><input type="text" name="ghichu" class="form-control" style="width: 600px;">
      ...
      <label> Status </label>
      <input type ="text" name = "status" value = "select" class = "form-control" style="width: 600px;" >
      <br />
...
<div>
 <?php
    ...
    for($i=0; $rowaz = $resultaz->fetch(); $i++){
      echo 'Mã : '.$rowaz['invoice_number'].'<br>';
      echo 'Ngày nhận: '.$rowaz['date_order'].'<br>';
      echo 'Loại bưu phẩm : '.$rowaz['suplier'].'<br>';
    ...
    <tbody>
     ...
     for($i=0; $row = $result->fetch(); $i++){
      ...
      $rrrrrrr1=$row['invoice'];
          $resultss1 = $db->prepare("SELECT * FROM purchases WHERE invoice_number= :asas1");
          $resultss1->bindParam(':asas1', $rrrrrrr1);
          $resultss1->execute();
          for($i=0; $rowss1 = $resultss1->fetch(); $i++){
            
            $from = $rowss1['nguoi_gui'];
            $invoice_number= $rowss1['invoice_number'];
          }
...
<!-- <?php
   include('connect.php');
          $result1 = $db->prepare("SELECT * FROM purchases ORDER BY transaction_id desc");
          $result1->execute();
          /*$nguoi_gui = $_GET['from'];*/
          ...
          <!--  <?php
               ...
               $result1 = $db->prepare("SELECT * FROM products WHERE product_name= :asas");
               $result1->bindParam(':asas', $macongty);
               $result1->execute();
...
<a href="Sendmail.php?name=<?php echo $macongty; ?>&email=<?php echo $mail;?>&nguoi_gui=<?php echo $from; ?>&status=<?php echo $status; ?>&invoice_number=<?php echo $invoice_number; ?>" class ="btn btn-primary"><span>Gửi mail thông báo</span></a>
</div>
<!-- <?php echo $a1;?>&nguoi_gui=<?php echo $nguoi_gui;?>&email=<?php echo $aa;?> -->
